        ...
        if content is not None:
            ...
            while(True):
                ...
                if i > 0:
                    ...
                    elif len(s_instances) == 0:
                        old_index = self.content[(i-1)//2]
                        for_choice = list(range(n - self.depth + 1))
                        for_choice.remove(old_index)
                        while len(s_instances) == 0:
                            print(for_choice)
                            replace_index = np.random.choice(for_choice, 1)[0]
                            self.content[(i-1)//2] = replace_index
                            s_instances = self.test_in_node(i)

                            if len(s_instances) != 0:
                                if i%2 == 1:
                                    s_instances = self.test_in_node(i+1)

                            if len(s_instances) == 0:
                                for_choice.remove(replace_index)
                        s_instances = self.test_in_node(i)
                        if len(s_instances) == 1:
                            self.content.append(str(s_instances[0][n]))
                            i += 1
